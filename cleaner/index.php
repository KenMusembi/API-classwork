...
";
$explode_=explode(",",$wambua_corrupted_employees);
var_dump($explode_);
die();
/*
$x=sizeof($explode);
...
for ($i=0; $i<$x; $i++)
{
  $tab=$tab."<tr>";
  for ($y=0; $y<8; $y++)
  $tab=$tab."<td>".$explode_[$i+ $y]."</td>"
}
echo $wambua_corrupted_employees;

 function extract_numbers($wambua_corrupted_employees)
 {
   preg_match_all('/([\d]+)/', $wambua_corrupted_employees,$match);
   return $match[0];
 }
 function extract_nonnumbers($wambua_corrupted_employees)
 {
   preg_match_all('#[^\d]#', $wambua_corrupted_employees,$match);
   return $match[0];
 }
 $number_array=extract_numbers($wambua_corrupted_employees);
 $nonnumbers_array=extract_nonnumbers($wambua_corrupted_employees);

 echo print_r($nonnumbers_array);
 echo print_r($number_array);
 ?>
*/
